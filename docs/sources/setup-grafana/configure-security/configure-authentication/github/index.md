...
| `org_roles_attribute_path`   | No       | [JMESPath](http://jmespath.org/examples.html) expression to use for Grafana org to role lookup. Grafana will first evaluate the expression using the user information obtained from the UserInfo endpoint. If no role is found, Grafana creates a JSON data with `groups` key that maps to GitHub teams obtained from GitHub's [`/api/user/teams`](https://docs.github.com/en/rest/teams/teams#list-teams-for-the-authenticated-user) endpoint, and evaluates the expression using this data. The result of the evaluation should be an array of objects having a `Role` key with a valid Grafana role as value (`Viewer`, `Editor`, `Admin` or `GrafanaAdmin`), and either an `OrgName` or `OrgID` key. For more information on user to org role mapping, refer to [Configure role mapping]({{< relref "#configure-role-mapping" >}}). |                                               |
...
Given a user member of the `@my-github-organization/my-github-team1` and `@my-github-organization/my-github-team2` Gitub teams.

In this example, the user has been granted the role of an `Editor` in the `@my-github-organization/my-github-team1` and `@my-github-organization/my-github-team2` orgs.
...
org_roles_attribute_path = groups[?starts_with(@, '@my-github-organization/')][{"OrgName": @, "Role": 'Editor'}][]
```
